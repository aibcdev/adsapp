...
export function AdminPayoutsPage() {
  ...
  const load = async (key: string, status: string) => {
    setBusy(true);
    setErr("");
    try {
      const [payoutRes, statsRes] = await Promise.all([
        adminFetch(`/v1/admin/payouts?status=${status}`, key),
  ...
  const mark = async (key: string, id: string, status: "paid" | "failed") => {
    await adminFetch(`/v1/admin/payouts/${id}`, key, {
      method: "PATCH",
...
function PayoutsInner({
  ...
  adminKey: string;
  tab: "requested" | "all";
  setTab: (t: "requested" | "all") => void;
  rows: PayoutRow[];
  stats: Stats | null;
  err: string;
  busy: boolean;
  load: (key: string, status: string) => Promise<void>;
  mark: (key: string, id: string, status: "paid" | "failed") => Promise<void>;
}) {
  useEffect(() => {
    void load(adminKey, tab);
  }, [adminKey, tab]);
          ...
          <li key={r.id} className="aibc-card flex flex-wrap items-center justify-between gap-4 p-4">
            ...
            {r.status === "requested" ? (
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => void mark(adminKey, r.id, "paid")}
                ...
                <button
                  type="button"
                  onClick={() => void mark(adminKey, r.id, "failed")}
