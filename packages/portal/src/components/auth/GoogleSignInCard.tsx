...
function GoogleSignInButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="flex w-full items-center justify-center gap-3 rounded-xl border border-zinc-200 bg-white py-3.5 text-sm font-semibold text-zinc-900 shadow-sm transition hover:bg-zinc-50 disabled:cursor-wait disabled:opacity-60"
    >
...
export function GoogleSignInCard({
  ...
  const sessionReady = Boolean(state);
  ...
  return (
    <div className={compact ? "aibc-card w-full p-6" : "aibc-card w-full max-w-md p-8"}>
      ...
      {!compact ? (
        <BrandHeading as="h1" className="mt-2 text-3xl text-zinc-950 md:text-4xl">
          {title}
        </BrandHeading>
      ) : null}
      {!compact ? <p className="mt-3 text-base font-medium text-zinc-600">{subtitle}</p> : null}

      {compact ? (
        <p className="mb-4 font-mono text-[10px] uppercase tracking-widest text-zinc-500">Sign in</p>
      ) : null}

      <GoogleSignInButton busy={busy} disabled={!sessionReady || busy} onClick={onGoogleSignIn} />

      {!sessionReady && !busy ? (
        <p className="mt-2 text-center text-xs text-zinc-400">Preparing secure sign-in…</p>
      ) : null}
